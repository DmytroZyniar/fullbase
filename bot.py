...
def format_result(result: LookupResult) -> str:
    lines = [
        f"📞 <b>Phone:</b> <code>{html.escape(result.phone)}</code>",
        f"👤 <b>Owner:</b> {html.escape(result.owner_name or 'N/A')}",
        f"📍 <b>Location:</b> {html.escape(result.location or 'N/A')}",
        "",
        "🔗 <b>Profiles / links:</b>",
    ]
    if result.social_profiles:
        lines.extend([f"- {html.escape(k)}: {html.escape(v)}" for k, v in result.social_profiles.items()])
    else:
        lines.append("- N/A")
    ...
    if result.contacts:
        lines.extend([f"- {html.escape(k)}: {html.escape(v)}" for k, v in result.contacts.items()])
    else:
        lines.append("- N/A")

    if result.notes:
        lines.append("")
        lines.append("ℹ️ <b>Notes:</b>")
...
async def run_search_by_type(update: Update, user_id: int, phone: str, search_type: str) -> None:
    ...
    await target.reply_text(format_result(result), parse_mode=ParseMode.HTML)
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("stype_"):
        if not limiter.allow(user_id):
            await query.message.reply_text(t(user_id, "rate_limited", db))
            return
        try:
        ...
        if not phone:
            await query.message.reply_text(t(user_id, "invalid_phone", db))
            return
